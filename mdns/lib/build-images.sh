#!/usr/bin/env bash
# Build mdns Docker images with caching (mirrors rendezvous/lib/build-images.sh)
...
prepare_vendored_github_source() {
  ...
  local commit_marker="${vendor_path}/.mdns-source-commit"
...
build_mdns_images() {
  ...
  for impl_id in "${impl_ids[@]}"; do
    ...
    build_mdns_image "${impl_id}" "${force_rebuild}" || {
      unindent
      return 1
    }
  done
