...
# Read from matrix
ADVERTISER_ID=$(yq eval ".${TEST_PASS}[${TEST_INDEX}].advertiser.id" "${TEST_PASS_DIR}/test-matrix.yaml")
ADVERTISER_IMAGE=$(yq eval ".${TEST_PASS}[${TEST_INDEX}].advertiser.imageName" "${TEST_PASS_DIR}/test-matrix.yaml")
...
COMPOSE_PROJECT_NAME="md_${TEST_SLUG}"
...
services:
  advertiser:
    image: "${ADVERTISER_IMAGE}"
    container_name: ${COMPOSE_PROJECT_NAME}_advertiser
    init: true
    environment:
      - ROLE=advertiser
      - REDIS_ADDR=mdns-redis:6379
      - TEST_KEY=${TEST_KEY}
