...
show_help() {
  cat <<HELP
libp2p mDNS Interoperability Test Runner
...
Examples:
  ${0}                                    # Run full test matrix (4 tests: go + py)
  ${0} --test-select "go_x_py"            # Run a single test
...
required_impl_filter_from_matrix() {
  ...
  local -n _out="$2"
  local ids=()
  local count test_id advertiser discoverer
  ...
  for ((i=0; i<count; i++)); do
    ...
    advertiser=$(yq eval ".tests[${i}].advertiser.id" "${matrix_file}")
    discoverer=$(yq eval ".tests[${i}].discoverer.id" "${matrix_file}")
    ids+=("${advertiser}" "${discoverer}")
  done
...
if [ "${LIST_IMAGES}" == "true" ]; then
  print_header "mDNS implementations"
  indent
...
build_mdns_images "${FORCE_IMAGE_REBUILD}" "${BUILD_FILTER}"

print_header "Starting global Redis..."
indent
docker rm -f mdns-redis 2>/dev/null || true
docker network create mdns-network 2>/dev/null || true
docker run -d --name mdns-redis --network mdns-network --rm redis:7-alpine >/dev/null
unindent

trap 'docker stop mdns-redis 2>/dev/null || true' EXIT
