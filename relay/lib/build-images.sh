#!/usr/bin/env bash
# Build relay Docker images with caching (mirrors kad-dht/lib/build-images.sh)
...
# Populate vendored github source into a local build context (e.g. go-libp2p-relay/)
...
prepare_vendored_github_source() {
  ...
  local commit_marker="${vendor_path}/.relay-source-commit"
...
build_relay_images() {
  ...
  for impl_id in "${impl_ids[@]}"; do
    ...
    build_relay_image "${impl_id}" "${force_rebuild}" || {
      unindent
      return 1
    }
  done
