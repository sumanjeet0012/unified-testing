...
show_help() {
  cat <<HELP
libp2p Relay Interoperability Test Runner

Usage: ${0} [options]

Filter Options:
  --impl-select VALUE       Select implementations to build (pipe-separated, e.g. "py|go")
...
Examples:
  ${0}                                    # Run full test matrix (8 tests: go + py)
  ${0} --test-select "go_x_py_x_go"       # Run a single test
  ${0} --force-image-rebuild              # Rebuild all Docker images
  ${0} --list-tests                       # List all generated test IDs
...
required_impl_filter_from_matrix() {
  ...
  local -n _out="$2"
  local ids=()
  local count test_id relay listener dialer
  ...
  for ((i=0; i<count; i++)); do
    ...
    relay=$(yq eval ".tests[${i}].relay.id" "${matrix_file}")
    listener=$(yq eval ".tests[${i}].listener.id" "${matrix_file}")
    dialer=$(yq eval ".tests[${i}].dialer.id" "${matrix_file}")
    ids+=("${relay}" "${listener}" "${dialer}")
  done
...
if [ "${LIST_IMAGES}" == "true" ]; then
  print_header "Relay implementations"
  indent
...
# Build only implementations required by selected tests
REQUIRED_IMPLS=""
BUILD_FILTER=""
...
build_relay_images "${FORCE_IMAGE_REBUILD}" "${BUILD_FILTER}"

print_header "Starting global Redis..."
indent
docker rm -f relay-redis 2>/dev/null || true
docker network create relay-network 2>/dev/null || true
docker run -d --name relay-redis --network relay-network --rm redis:7-alpine >/dev/null
unindent

trap 'docker stop relay-redis 2>/dev/null || true' EXIT
