...
show_help() {
  cat <<HELP
libp2p Rendezvous Interoperability Test Runner
...
required_impl_filter_from_matrix() {
  ...
  local -n _out="$2"
  local ids=()
  local count test_id server registrant discoverer
  ...
  for ((i=0; i<count; i++)); do
    ...
    server=$(yq eval ".tests[${i}].server.id" "${matrix_file}")
    registrant=$(yq eval ".tests[${i}].registrant.id" "${matrix_file}")
    discoverer=$(yq eval ".tests[${i}].discoverer.id" "${matrix_file}")
    ids+=("${server}" "${registrant}" "${discoverer}")
  done
...
if [ "${LIST_IMAGES}" == "true" ]; then
  print_header "Rendezvous implementations"
  indent
...
build_rendezvous_images "${FORCE_IMAGE_REBUILD}" "${BUILD_FILTER}"
