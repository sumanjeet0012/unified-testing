...
def _clean_transport_addr(addr: Multiaddr) -> Multiaddr:
    """Decapsulate /p2p/ peer suffix from transport multiaddr if present."""
    try:
        p2p_val = addr.value_for_protocol("p2p")
        if p2p_val:
            return addr.decapsulate(Multiaddr(f"/p2p/{p2p_val}"))
...
def create_register_message(
    ...
    for addr in addrs:
        peer_info.addrs.append(_clean_transport_addr(addr).to_bytes())
...
def parse_peer_info(peer_info: Message.PeerInfo) -> tuple[PeerID, list[Multiaddr]]:
    ...
    addrs = [_clean_transport_addr(Multiaddr(addr_bytes)) for addr_bytes in peer_info.addrs]
