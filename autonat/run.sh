...
show_help() {
  cat <<HELP
libp2p AutoNAT Interoperability Test Runner
...
required_impl_filter_from_matrix() {
  ...
  local -n _out="$2"
  local ids=()
  local count test_id server client
  ...
  for ((i=0; i<count; i++)); do
    ...
    client=$(yq eval ".tests[${i}].client.id" "${matrix_file}")
    ids+=("${server}" "${client}")
  done
...
if [ "${LIST_IMAGES}" == "true" ]; then
  print_header "AutoNAT implementations"
  indent
...
build_autonat_images "${FORCE_IMAGE_REBUILD}" "${BUILD_FILTER}"
