#!/usr/bin/env bash
# Build bootstrap Docker images with caching (mirrors rendezvous/lib/build-images.sh)
...
prepare_vendored_github_source() {
  ...
  local commit_marker="${vendor_path}/.bootstrap-source-commit"
...
build_bootstrap_images() {
  ...
  for impl_id in "${impl_ids[@]}"; do
    ...
    build_bootstrap_image "${impl_id}" "${force_rebuild}" || {
      unindent
      return 1
    }
  done
