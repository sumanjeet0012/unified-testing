...
show_help() {
  cat <<HELP
libp2p Bootstrap Interoperability Test Runner
...
required_impl_filter_from_matrix() {
  ...
  local -n _out="$2"
  local ids=()
  local count test_id joiner bootstrap
  ...
  for ((i=0; i<count; i++)); do
    ...
    joiner=$(yq eval ".tests[${i}].joiner.id" "${matrix_file}")
    bootstrap=$(yq eval ".tests[${i}].bootstrap.id" "${matrix_file}")
    ids+=("${joiner}" "${bootstrap}")
  done
...
if [ "${LIST_IMAGES}" == "true" ]; then
  print_header "Bootstrap implementations"
  indent
...
build_bootstrap_images "${FORCE_IMAGE_REBUILD}" "${BUILD_FILTER}"

print_header "Starting global Redis..."
indent
docker rm -f bootstrap-redis 2>/dev/null || true
docker network create bootstrap-network 2>/dev/null || true
docker run -d --name bootstrap-redis --network bootstrap-network --rm redis:7-alpine >/dev/null
unindent

trap 'docker stop bootstrap-redis 2>/dev/null || true' EXIT
